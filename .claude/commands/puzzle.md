...
1. `GET https://minohlee.mooo.com/api/puzzles`를 호출해 오늘 날짜(YYYY-MM-DD)의 퍼즐 수 확인
...
퍼즐 JSON을 구성한 뒤 **원격 서버** `https://minohlee.mooo.com/api/puzzles`에 저장합니다.
...
Bash로 저장:
```bash
curl -s -X POST https://minohlee.mooo.com/api/puzzles \
...
게임 링크: https://minohlee.mooo.com/puzzle/play?id={puzzle-id}
퍼즐 목록: https://minohlee.mooo.com/puzzle
```
