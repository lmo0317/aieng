...
### Phase 4: Layout Design (그리드 설계 - 다양성 극대화)
- **🚨 중요: 정형화된 패턴 탈피**: 매번 (1,1)에서 시작하거나 단순한 계단식 배열을 반복하지 마십시오. **완전히 새로운 구조**를 설계하십시오.
- **배치 규칙**:
    1. **시작점 무작위화**: 첫 번째 단어의 시작 위치(startX, startY)를 그리드 중앙이나 임의의 지점에서 시작하여 사방으로 뻗어 나가게 하십시오.
    2. **교차 패턴 다양화**: 
        - **중심 교차**: 하나의 긴 단어에 여러 개의 짧은 단어가 수직으로 교차하는 구조.
        - **연쇄 교차**: 단어 끄트머리가 서로 맞물리며 길게 이어지는 구조.
        - **독립 군집**: 여러 개의 교차된 덩어리들이 그리드 곳곳에 배치되는 구조.
    3. **그리드 공간 활용**: 단어들이 그리드 한쪽에 쏠리지 않도록 상하좌우 여백을 충분히 활용하여 넓게 배치하십시오.
    4. **인접 금지**: 교차점 외에는 다른 단어와 상하좌우로 붙어있으면 안 됨 (플레이어가 혼란을 겪지 않도록 엄격히 준수).
- **그리드 크기**: 
    - count=3: 10×10 권장 (공간 확보)
    - count=4: 12×12 권장
    - count=5: 14×14 최대 (다양한 배치를 위해 약간 확장 허용)
...
2. **단어 중복 엄금**: 기존 퍼즐에서 사용된 단어(`answer`)를 절대 재사용하지 마십시오.
3. **레이아웃 독창성**: 이전 퍼즐과 동일하거나 매우 유사한(단순 계단식 등) 레이아웃을 반복하지 마십시오. 매번 **완전히 새로운 무작위성**을 부여하십시오.
4. **교차 검증**: 가로 단어와 세로 단어가 만나는 지점의 글자가 반드시 일치하는지 확인하십시오.
