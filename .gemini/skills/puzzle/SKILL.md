...
1. **교차점 외 인접 금지 (No Adjacency)**: 교차점(글자가 겹치는 칸)을 제외하고는 **어떠한 단어의 칸도 상하좌우로 서로 붙어있으면 안 됩니다.** 단어 사이에는 반드시 최소 한 칸 이상의 빈 공간이 있어야 합니다.
2. **교차점 글자 일치 (Zero Conflict)**: 가로와 세로 단어가 만나는 지점의 글자가 반드시 일치해야 합니다. **전수 조사(Phase 4)** 없이는 절대로 저장하지 마십시오.
3. **단어 중복 금지**: 이전에 생성된 퍼즐에서 사용된 단어는 다시 사용하지 마십시오. 반드시 사전에 확인하십시오.
4. **레이아웃 독창성**: 단순 계단식 배열을 반복하지 마십시오. 매번 새로운 무작위성을 부여하십시오.
...
### Phase 4: Grid Verification Protocol (교차점 및 인접 무결성 검증 - 필수)
- **전수 조사**: 퍼즐을 확정하기 전, 다음 두 가지를 100% 확인하십시오.
    1. **교차점 일치**: 모든 교차 좌표의 글자가 두 단어 모두에서 동일한가?
    2. **인접 여부**: 교차점을 제외한 모든 단어의 칸들 상하좌우에 다른 단어의 칸이 붙어있지 않은가? (빈 공간 확보 확인)
- **오류 발견 시**: 즉시 좌표를 수정하거나 단어를 교체하여 문제를 해결하십시오. **규칙 위반 퍼즐은 절대로 저장하지 마십시오.**
...
2. **인접 엄금**: 교차점 외에 단어끼리 상하좌우로 맞붙는 레이아웃은 허용되지 않습니다. (최소 한 칸의 여백 필수)
3. **교차점 글자 일치**: 가로와 세로 단어가 만나는 지점의 글자가 100% 일치해야 합니다. (불일치 시 저장 금지)
4. **단어 중복 엄금**: 기존 퍼즐에서 사용된 단어(`answer`)를 절대 재사용하지 마십시오.
5. **레이아웃 독창성**: 이전 퍼즐과 동일하거나 매우 유사한(단순 계단식 등) 레이아웃을 반복하지 마십시오. 매번 **완전히 새로운 무작위성**을 부여하십시오.
